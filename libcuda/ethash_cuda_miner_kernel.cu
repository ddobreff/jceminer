...
)
{
	...
	if (!compute_hash<_PARALLEL_HASH>(start_nonce + gid, d_target, mix))
		return;
	if (atomicInc((unsigned*)&g_output->count, 0xffffffff))
		return;
	g_output->gid = gid;
...
__global__ void
ethash_calculate_dag_item(uint32_t start, bool check)
{
	...
	for (uint32_t t = 0; t < 4; t++) {
		...
		else if (!(shuffle_index == 0 && thread_id < 2))
			dag_nodes[shuffle_index].uint4s[thread_id] = s[thread_id];
	}
}
...
)
{
	...
	const uint32_t chunk = blocks * threads;
	for (uint32_t i = 0; i < work; i += chunk) {
		ethash_calculate_dag_item <<< blocks, threads, 0, stream>>>(i, i != 0);
